...
def train(epoches=1):
    encoder = Encoder(4000, 200, 256, 2)
    decoder = Decoder(5000, 200, 256, 4000, 2)
    ...
    for epoch in range(epoches):
        ...
        for encoder_inputs, decoder_inputs, decoder_tars in process_data.get_batchdata(en_train_data, 3000, cn_train_data, 4000, batch_size=batch_size):
            ...
            for i in range(decoder_tars.size(1)):
                ...
                decoder_outputs, hidden = decoder(decoder_inputs[:, i].unsqueeze(1), hidden)
        ...
        if epoch %100==0:
            encoder_model_path = './model/'+'encoder'+ str(epoch)+'_loss_'+ str(total_loss) + '.pkl'
            torch.save(encoder, encoder_model_path)
            decoder_model_path = './model/'+'decoder' + str(epoch) + '_loss_' + str(total_loss) + '.pkl'
